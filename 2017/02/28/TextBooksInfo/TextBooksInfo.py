...
if __name__ == '__main__':
    ...
    # 学期
    semester = date.today().year-2017+102
    if date.today().month < 9 and date.today().month > 2:
        semester+=1
    # 学生id
    now = str(time.time()).replace('.', '1')
    res = s.get('http://4m3.tongji.edu.cn/eams/courseTableForStd.action?_='+now).text.encode('utf-8')
    open('b.html','w').write(res)
    id = re.search('"ids",\"(.*?)"\)\;', res)
